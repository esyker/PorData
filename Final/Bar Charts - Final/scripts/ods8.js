...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "8" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
...
change_Arrow_Paginate_Color("#A21942");
}
...
if (d.Title == 'PIB real <em>per capita</em> (Euro)') {
    return d;
}
});
...
const yLabelSize = BrowserText.getWidth('Milhares de Euros', 14, 'sans-serif');
...
data = data.filter(function (d) {
  if (d.Title == 'Investimento: Formação bruta de capital fixo em % do PIB') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('% do PIB', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Investimento: Formação bruta de capital fixo em % do PIB");
...
data = data.filter(function (d) {
  if (d.Title == 'Taxa de jovens não empregados que não estão em educação ou formação: total e por sexo') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Taxa de jovens não empregados que não estão em educação ou formação");
}  
...
data = data.filter(function (d) {
  if (d.Title == 'Taxa de emprego: total e por sexo') {
      return d;
  }
});
...
data = data.filter(function (d) {
  if (d.Title == 'Taxa de desemprego de longa duração: total e por sexo') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Taxa de desemprego de longa duração");
...
data = data.filter(function (d) {
  if (d.Title == 'Acidentes de trabalho mortais por 100 mil empregados: total e por sexo') {
      return d;
  }
});
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Acidentes de trabalho mortais por 100 mil empregados");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "Taxa - %000" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "% do PIB" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Milhares de Euros" + ": " +"</strong> "  +  d.Value/1000 )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
