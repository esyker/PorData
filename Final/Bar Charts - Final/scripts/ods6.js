...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "6" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
  ...
  for (let i = 0; i < nlist.length; i++) {
    ...
    if (nlist[i].className.includes('active')) {
      ...
      else if(div_ativa == "#my_dataviz3")
      {
        ...
        cleanSelect("my_select");
        var years_valid = [2017,2016,2015,2014,2013,2012,2011,2010]
        populateSelect("my_select", years_valid);
        sortSelect("my_select");
        $('#my_select').val(years_valid[0]);
...
change_Arrow_Paginate_Color("#26BDE2");
}
...
data = data.filter(function (d) {
  if (d.Title == 'População que vive sem banheira, duche e retrete no interior do alojamento') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("População que vive sem banheira, duche e retrete no interior do alojamento");
...
data = data.filter(function (d) {
  if (d.Title == 'População servida pelo menos por sistemas de tratamento secundário de águas residuais (%)') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("População servida pelo menos por sistemas de tratamento secundário de águas residuais (%)");
}  
...
data = data.filter(function (d) {
  if (d.Title == 'Carência bioquímica de oxigénio nos rios') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('mg O2 / L', 14, 'sans-serif');
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Carência bioquímica de oxigénio nos rios");
...
data = data.filter(function (d) {
  if (d.Title == 'Nitrato nas águas subterrâneas') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('mg NO3 / L', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Nitrato nas águas subterrâneas");
...
const yLabelSize = BrowserText.getWidth('PO4 / L', 14, 'sans-serif');
...
data = data.filter(function (d) {
if (d.Title == 'Índice de exploração da água (Wei +)') {
    return d;
}
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Índice de exploração da água (Wei +)");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "mg O2 / L" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "PO4 / L" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "mg NO3 / L" + ": " +"</strong> "  +  d.Value )      
      .style('left', `${pos['x']}px`)
      .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
