...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "10" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
...
change_Arrow_Paginate_Color("#DD1367");
}
...
if (d.Title == 'PIB <em>per capita</em> (PPS)') {
    return d;
}
});
...
const yLabelSize = BrowserText.getWidth('PPS - Rácio', 14, 'sans-serif');
...
data = data.filter(function (d) {
  if (d.Title == 'Rendimento disponível bruto das famílias <em>per capita</em> (PPS)') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Rendimento disponível bruto das famílias 'per capita' (PPS)");
...
data = data.filter(function (d) {
  if (d.Title == 'Taxa de intensidade da pobreza') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Taxa de intensidade da pobreza");
}  
...
data = data.filter(function (d) {
  if (d.Title == 'Desigualdade na distribuição do rendimento (S80/S20)') {
      return d;
  }
});
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Desigualdade na distribuição do rendimento (S80/S20)");
...
data = data.filter(function (d) {
  if (d.Title == 'Proporção de rendimento dos 40% mais pobres da população') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Proporção de rendimento dos 40% mais pobres da população");
...
data = data.filter(function (d) {
  if (d.Title == 'Pedidos de asilo por milhão de habitantes') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Rácio %0000 - Milhares', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Pedidos de asilo por milhão de habitantes");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Rácio %0000" + ": " +"</strong> "  +  d.Value/1000 + "k")      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "PPS - Rácio" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
