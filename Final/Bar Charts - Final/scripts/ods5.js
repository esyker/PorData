...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "5" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
  ...
  for (let i = 0; i < nlist.length; i++) {
    ...
    if (nlist[i].className.includes('active')) {
      ...
      else if(div_ativa == "#my_dataviz3")
      {
        ...
        cleanSelect("my_select");
        var years_valid = [2020,2019,2018,2017,2016,2015,2014,2013,2012,2011,2010]
        populateSelect("my_select", years_valid);
        sortSelect("my_select");
        $('#my_select').val(years_valid[0]);
...
change_Arrow_Paginate_Color("#FF3A21");
}
...
if (d.Title == 'Mulheres vítimas de violência física e sexual') {
    return d;
}
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Mulheres vítimas de violência física e sexual");
...
data = data.filter(function (d) {
  if (d.Title == 'Disparidade salarial entre homens e mulheres') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Taxa - %', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Disparidade salarial entre homens e mulheres");
...
data = data.filter(function (d) {
  if (d.Title == 'Disparidade entre homens e mulheres no emprego') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Pontos Percentuais', 14, 'sans-serif');
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Disparidade entre homens e mulheres no emprego");
...
data = data.filter(function (d) {
  if (d.Title == 'População inactiva devido a responsabilidades familiares: total e por sexo') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("População inactiva devido a responsabilidades familiares: total e por sexo");
...
data = data.filter(function (d) {
  if (d.Title == 'Proporção de mulheres em conselhos de administração') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Proporção de mulheres em conselhos de administração");
...
data = data.filter(function (d) {
  if (d.Title == 'Proporção de mulheres executivas') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Proporção de mulheres executivas");
}  
...
data = data.filter(function (d) {
  if (d.Title == 'Proporção de mulheres nos governos nacionais') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Proporção de mulheres nos governos nacionais");
}  
...
data = data.filter(function (d) {
  if (d.Title == 'Proporção de mulheres nos parlamentos') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Proporção de mulheres nos parlamentos");
}  
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "Pontos Percentuais" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Taxa - %" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
