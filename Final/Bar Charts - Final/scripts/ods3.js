...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "3" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
  ...
  for (let i = 0; i < nlist.length; i++) {
    ...
    if (nlist[i].className.includes('active')) {
      ...
      else if(div_ativa == "#my_dataviz3")
      {
        ...
        cleanSelect("my_select");
        var years_valid = [2020,2017,2014,2012]
        populateSelect("my_select", years_valid);
        sortSelect("my_select");
        $('#my_select').val(years_valid[0]);
...
change_Arrow_Paginate_Color("#4C9F38");
}
...
data = data.filter(function (d) {
  if (d.Title == 'Esperança de vida à nascença: total e por sexo') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Anos', 14, 'sans-serif');
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Esperança de vida à nascença");
...
data = data.filter(function (d) {
  if (d.Title == 'População com boa ou muito boa percepção do seu estado de saúde: total e por sexo') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Proporção % 16+', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("População com boa ou muito boa percepção do seu estado de saúde");
...
data = data.filter(function (d) {
  if (d.Title == 'Prevalência do tabagismo: total e por sexo') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Proporção % 15+', 14, 'sans-serif');
...
data = data.filter(function (d) {
  if (d.Title == 'Taxa de mortalidade padronizada por tuberculose, VIH e hepatite') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Pessoas / 100 mil habitantes', 14, 'sans-serif');
...
.attr("y", 20)
.attr("x", -(height - margin.top - margin.bottom - yLabelSize)/2)
.text("Pessoas / 100 mil habitantes")
.style("fill", "#000033")
.style("font-size", "14px")
.attr("font-family", "sans-serif")
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Taxa de mortalidade padronizada por tuberculose, VIH e hepatite");
}  
...
data = data.filter(function (d) {
  if (d.Title == 'Taxa de mortalidade padronizada evitável') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Taxa de mortalidade padronizada evitável");
...
data = data.filter(function (d) {
if (d.Title == 'População com necessidades insatisfeitas de cuidados médicos: total e por sexo') {
    return d;
}
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("População com necessidades insatisfeitas de cuidados médicos");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "Anos" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Proporção % 16+" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Proporção % 15+" + ": " +"</strong> "  +  d.Value )      
      .style('left', `${pos['x']}px`)
      .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
