...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "12" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
  ...
  for (let i = 0; i < nlist.length; i++) {
    ...
    if (nlist[i].className.includes('active')) {
      ...
      else if(div_ativa == "#my_dataviz4")
      {
        ...
        cleanSelect("my_select");
        var years_valid = [2018,2016,2014,2012,2010]
        populateSelect("my_select", years_valid);
        sortSelect("my_select");
        $('#my_select').val(years_valid[0]);
...
change_Arrow_Paginate_Color("#BF8B2E");
}
...
if (d.Title == 'Produtividade de recursos e consumo interno de materiais (PPS)') {
    return d;
}
});
...
const yLabelSize = BrowserText.getWidth('PPS / kg', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Produtividade de recursos e consumo interno de materiais (PPS)");
...
data = data.filter(function (d) {
  if (d.Title == 'Emissão média de CO<sub>2</sub> por km dos automóveis novos de passageiros') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('g CO2 / km', 14, 'sans-serif');
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Emissão média de CO2 por km dos automóveis novos de passageiros");
...
data = data.filter(function (d) {
  if (d.Title == 'Taxa de utilização de material circular') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Taxa de utilização de material circular");
...
data = data.filter(function (d) {
  if (d.Title == 'Produção de resíduos, excluindo os principais resíduos minerais') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth("kg 'per capita'", 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Produção de resíduos, excluindo os principais resíduos minerais");
...
data = data.filter(function (d) {
  if (d.Title == 'Valor acrescentado bruto dos bens e serviços ambientais em % do PIB') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Valor acrescentado bruto dos bens e serviços ambientais em % do PIB");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "g CO2 / km" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "kg 'per capita'" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "PPS / kg" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
