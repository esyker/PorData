...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "17" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
...
change_Arrow_Paginate_Color("#19486A");
}
...
data = data.filter(function (d) {
  if (d.Title == 'Ajuda pública ao desenvolvimento em % do rendimento nacional bruto') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Ajuda pública ao desenvolvimento em % do rendimento nacional bruto");
...
data = data.filter(function (d) {
  if (d.Title == 'Financiamento da União Europeia aos países em desenvolvimento') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Financiamento da União Europeia aos países em desenvolvimento");
...
data = data.filter(function (d) {
  if (d.Title == 'Importações da União Europeia de países em desenvolvimento') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Importações da União Europeia de países em desenvolvimento");
...
data = data.filter(function (d) {
if (d.Title == 'Administrações Públicas: dívida bruta em % do PIB') {
    return d;
}
});
...
const yLabelSize = BrowserText.getWidth('Dívida bruta em % do PIB', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Administrações Públicas: dívida bruta em % do PIB");
...
data = data.filter(function (d) {
if (d.Title == 'Contribuição dos impostos ambientais nas receitas fiscais totais (%)') {
    return d;
}
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Contribuição dos impostos ambientais nas receitas fiscais totais (%)");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Milhões de Euros" + ": " +"</strong> "  +  d.Value )      
      .style('left', `${pos['x']}px`)
      .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
