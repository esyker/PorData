...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "4" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
...
change_Arrow_Paginate_Color("#C5192D");
}
...
data = data.filter(function (d) {
  if (d.Title == 'Taxa de abandono precoce de educação e formação: total e por sexo') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('% da população 18-24', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Taxa de abandono precoce de educação e formação: total e por sexo");
...
data = data.filter(function (d) {
  if (d.Title == 'População, entre os 30 e os 34 anos, com o ensino superior (ISCED 5-8) (%)') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("População, entre os 30 e os 34 anos, com o ensino superior");
...
data = data.filter(function (d) {
if (d.Title == 'Participação no ensino pré-escolar') {
    return d;
}
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Participação no ensino pré-escolar");
...
data = data.filter(function (d) {
  if (d.Title == 'Insucesso na leitura, matemática e ciências') {
      return d;
  }
});
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Insucesso na leitura, matemática e ciências");
...
data = data.filter(function (d) {
  if (d.Title == 'Participação de adultos na aprendizagem') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Proporção % 25-64', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Participação de adultos na aprendizagem");
...
data = data.filter(function (d) {
  if (d.Title == 'Taxa de emprego dos recém-diplomados com pelo menos o ensino secundário') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Taxa de emprego dos recém-diplomados com pelo menos o ensino secundário");
}  
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "Proporção %" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Proporção % 25-64" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "% da população 18-24" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
