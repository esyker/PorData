...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "2" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
...
change_Arrow_Paginate_Color("#DDA63A");
}
...
const yLabelSize = BrowserText.getWidth('% da População 18+', 14, 'sans-serif');
...
data = data.filter(function (d) {
  if (d.Title == 'Rendimento agrícola dos factores por unidade de trabalho ano (UTA)') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Rendimento por UTA', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Rendimento agrícola dos factores por unidade de trabalho ano (UTA)");
...
data = data.filter(function (d) {
  if (d.Title == 'Apoio do Governo à investigação e desenvolvimento (I&D) na agricultura <em>per capita</em>') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('€ per capita', 14, 'sans-serif');
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Apoio do Governo à investigação e desenvolvimento (I&D) na agricultura");
...
data = data.filter(function (d) {
  if (d.Title == 'Área em agricultura biológica') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('% da superfície agrícola utilizada ', 14, 'sans-serif');
...
.attr("y", 20)
.attr("x", -(height - margin.top - margin.bottom - yLabelSize)/2)
.text("% da superfície agrícola utilizada ")
.style("fill", "#000033")
.style("font-size", "14px")
.attr("font-family", "sans-serif")
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Área de Cultivo Biológico");
...
data = data.filter(function (d) {
  if (d.Title == 'Indicador de risco harmonizado de pesticidas (IRH1)') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Risco Harmonizado', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Indicador de risco harmonizado de pesticidas (IRH1)");
...
data = data.filter(function (d) {
if (d.Title == 'Emissões de amoníaco pela agricultura') {
    return d;
}
});
...
const yLabelSize = BrowserText.getWidth('kg NH3 / ha', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Emissões de amoníaco pela agricultura");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "€ <i>per capita</i>" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "% da SAU" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "kg NH3 / ha" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Risco Harmonizado" + ": " +"</strong> "  +  d.Value )      
      .style('left', `${pos['x']}px`)
      .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Rendimento" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " +  "<strong>" + "<br>" + "% da população 18+" + " : " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
}
