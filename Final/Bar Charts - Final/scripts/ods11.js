...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "11" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
  ...
  for (let i = 0; i < nlist.length; i++) {
    ...
    if (nlist[i].className.includes('active')) {
      ...
      else if(div_ativa == "#my_dataviz3")
      {
        ...
        cleanSelect("my_select");
        var years_valid = [2018,2015,2012]
        populateSelect("my_select", years_valid);
        sortSelect("my_select");
        $('#my_select').val(years_valid[0]);
...
change_Arrow_Paginate_Color("#FD9D24");
}
...
data = data.filter(function (d) {
  if (d.Title == 'Taxa de sobrelotação da habitação') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Taxa de sobrelotação da habitação");
...
data = data.filter(function (d) {
  if (d.Title == 'Proporção da população que vive em agregados familiares e que refere a existência de ruído') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Proporção da população que vive em agregados familiares e que refere a existência de ruído");
...
data = data.filter(function (d) {
  if (d.Title == 'Territórios artificializados e áreas de suporte às actividades humanas <em>per capita</em>') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Metros Quadrados - Rácio', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Territórios artificializados e áreas de suporte às actividades humanas 'per capita'");
...
data = data.filter(function (d) {
  if (d.Title == 'Vítimas mortais em acidentes de viação por 100 mil residentes') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Vítimas mortais em acidentes de viação por 100 mil residentes");
}  
...
data = data.filter(function (d) {
  if (d.Title == 'Exposição a poluição atmosférica por partículas inaláveis') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('PM10 Rácio', 14, 'sans-serif');
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Exposição a poluição atmosférica por partículas inaláveis");
...
data = data.filter(function (d) {
if (d.Title == 'Taxa de reciclagem de resíduos municipais') {
    return d;
}
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Taxa de reciclagem de resíduos municipais");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "PM10 Rácio" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "m"+"<sup>2</sup>"+ " Rácio" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
