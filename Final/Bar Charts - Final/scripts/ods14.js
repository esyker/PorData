...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "14" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
...
change_Arrow_Paginate_Color("#0A97D9");
}
...
data = data.filter(function (d) {
  if (d.Title == 'Superfície de sítios marinhos designados ao abrigo da rede Natura 2000') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Km Quadrados', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Superfície de sítios marinhos designados ao abrigo da rede Natura 2000");
...
data = data.filter(function (d) {
  if (d.Title == 'Áreas balneares com qualidade de água excelente') {
      return d;
  }
});
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Áreas balneares com qualidade de água excelente");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "km<sup>2</sup>" + ": " +"</strong> "  +  d.Value/1000 + "k" )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
