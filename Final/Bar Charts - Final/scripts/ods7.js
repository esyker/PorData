...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "7" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
...
change_Arrow_Paginate_Color("#FCC30B");
}
...
data = data.filter(function (d) {
  if (d.Title == 'Consumo de energia primária') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('ton equivalente de petróleo', 14, 'sans-serif');
...
.text("ton equivalente de petróleo")
.style("fill", "#000033")
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Consumo de energia primária");
...
data = data.filter(function (d) {
  if (d.Title == 'Consumo de energia final das famílias <em>per capita</em>') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('kg equivalente de petróleo', 14, 'sans-serif');
...
svg.append("text")
  ...
  .attr("y", 15)
  .attr("x", -(height - margin.top - margin.bottom - yLabelSize)/2)
  .text("kg equivalente de petróleo")
  .style("fill", "#000033")
  .style("font-size", "14px")
  .attr("font-family", "sans-serif")
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Consumo de energia final das famílias 'per capita'");
...
data = data.filter(function (d) {
  if (d.Title == 'Produtividade da energia (Euro)') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Euro / kg equivalente de petróleo', 14, 'sans-serif');
...
.attr("y", 20)
.attr("x", -(height - margin.top - margin.bottom - yLabelSize)/2)
.text("Euro / kg equivalente de petróleo")
.style("fill", "#000033")
.style("font-size", "14px")
.attr("font-family", "sans-serif")
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Produtividade da energia (Euro)");
}  
...
data = data.filter(function (d) {
if (d.Title == 'Contribuição das energias renováveis no consumo de energia final (%)') {
    return d;
}
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Contribuição das energias renováveis no consumo de energia final");
...
data = data.filter(function (d) {
  if (d.Title == 'Dependência das importações de energia') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Dependência das importações de energia");
...
data = data.filter(function (d) {
  if (d.Title == 'População incapaz de aquecer convenientemente a habitação') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("População incapaz de aquecer convenientemente a habitação");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "kgep" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "tep" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Rácio %" + ": " +"</strong> "  +  d.Value )      
      .style('left', `${pos['x']}px`)
      .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " +  "<strong>" + "<br>" + "Euro / kgep" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
}
