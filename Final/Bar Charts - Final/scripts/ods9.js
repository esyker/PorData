...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "9" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
...
change_Arrow_Paginate_Color("#FD6925");
}
...
data = data.filter(function (d) {
  if (d.Title == 'Despesas em actividades de investigação e desenvolvimento (I&D) em % do PIB') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Rácio - %', 14, 'sans-serif');
...
svg.append("text")
  ...
  .style("fill", "#3A3B3C")
  .attr("font-family", "sans-serif")
  .text("Despesas em actividades de investigação e desenvolvimento (I&D) em % do PIB");
...
data = data.filter(function (d) {
if (d.Title == 'Pessoal em actividades de investigação e desenvolvimento (I&D): equivalente a tempo integral em % da população activa') {
    return d;
}
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Pessoal em actividades de investigação e desenvolvimento (I&D)");
...
data = data.filter(function (d) {
  if (d.Title == 'Pedidos de patentes ao Instituto Europeu de Patentes (IEP) por milhão de habitantes') {
      return d;
  }
});
...
const yLabelSize = BrowserText.getWidth('Rácio %0000', 14, 'sans-serif');
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Pedidos de patentes ao Instituto Europeu de Patentes (IEP) por milhão de habitantes");
...
data = data.filter(function (d) {
  if (d.Title == 'Proporção de passageiros transportados por autocarros e comboios') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Proporção de passageiros transportados por autocarros e comboios");
...
data = data.filter(function (d) {
  if (d.Title == 'Proporção de mercadorias transportadas por ferrovia e navegação interior') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Proporção de mercadorias transportadas por ferrovia e navegação interior");
...
data = data.filter(function (d) {
  if (d.Title == 'Recursos humanos em ciência e tecnologia em % da população activa') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Recursos humanos em ciência e tecnologia em % da população activa");
}  
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "Rácio - %" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<strong>" + "<br>" + "Rácio %0000" + ": " +"</strong> "  +  d.Value )      
    .style('left', `${pos['x']}px`)
    .style('top', `${(window.pageYOffset  + pos['y'] - 40)}px`)
