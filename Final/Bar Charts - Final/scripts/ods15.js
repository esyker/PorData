...
function init() {
  ...
  d3.csv(data_file).then((data) =>{
    ...
    dataSet = data.filter(function (d) {
      if (d.ODS == "15" && d.Símbolo != "Valor não disponível"){
        return d;
      }
    });
  ...
  for (let i = 0; i < nlist.length; i++) {
    ...
    if (nlist[i].className.includes('active')) {
      ...
      else if(div_ativa == "#my_dataviz2")
      {
        ...
        cleanSelect("my_select");
        var years_valid = [2019,2018,2017,2016,2015,2014,2013,2012,2011]
        populateSelect("my_select", years_valid);
        sortSelect("my_select");
        $('#my_select').val(years_valid[0]);
...
change_Arrow_Paginate_Color("#56C02B");
}
...
data = data.filter(function (d) {
  if (d.Title == 'Superfície terrestre dos Sítios designados no âmbito da Rede Natura 2000') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Superfície terrestre dos Sítios designados no âmbito da Rede Natura 2000");
...
data = data.filter(function (d) {
if (d.Title == 'Índice de impermeabilização dos solos') {
    return d;
}
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Índice de impermeabilização dos solos");
...
data = data.filter(function (d) {
  if (d.Title == 'Erosão hídrica dos solos estimada - taxa de área afetada por erosão severa') {
      return d;
  }
});
...
.style("fill", "#3A3B3C")
.attr("font-family", "sans-serif")
.text("Erosão hídrica dos solos estimada - taxa de área afetada por erosão severa");
...
function handleMouseOver(event, d) {
  ...
  div.html("<strong>" + d.Country + "</strong> " + "<br>" + "<strong>" + "Proporção %" + ": " +"</strong> ")      
    .style('left', `${pos['x']}px`)
